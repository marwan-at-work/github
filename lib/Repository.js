...
class Repository extends Requestable {
    ...
    */
   constructor(user, repo, auth, apiBase) {
      super(auth, apiBase);
      this.__user = user;
      this.__repo = repo;
      this.__fullname = repo ? `${user}/${repo}` : `${user}`;
      this.__currentTree = {
         branch: null,
         sha: null
      };
   ...
   /**
    * Add collaborator to this repo.
    * @see https://developer.github.com/v3/repos/collaborators/#add-user-as-a-collaborator
    * @param {Object} options - options with username as collaborator - permission (optional)
    * @param {Requestable.callback} [cb] - will receive the list of PRs
    * @return {Promise} - the promise for the http request
    */
   addCollaborator(options, cb) {
      options = options || {};
      const payload = options.permission ? {permission: options.permission} : null;

      return this._request('PUT', `/repos/${this.__fullname}/collaborators/${options.username}`, payload, cb);
   }

   /**
    * Remove collaborator to this repo.
    * @see https://developer.github.com/v3/repos/collaborators/#remove-user-as-a-collaborator
    * @param {Object} options - options with username as collaborator
    * @param {Requestable.callback} [cb] - will receive the list of PRs
    * @return {Promise} - the promise for the http request
    */
   removeCollaborator(options, cb) {
      options = options || {};

      return this._request('DELETE', `/repos/${this.__fullname}/collaborators/${options.username}`, null, cb);
   }
